...
export default function HeroCarousel({ locale }: HeroCarouselProps) {
  ...
  return (
    <div className="relative h-[700px] md:h-[800px] overflow-hidden group">
      ...
      <div className="absolute bottom-0 left-0 w-80 h-80 bg-gradient-to-tr from-[#ff0000]/5 to-transparent rounded-full transform -translate-x-40 translate-y-40"></div>
      ...
      {slides.map((slide, index) => (
        ...
        >
          ...
          >
            ...
            <div className="absolute inset-0 bg-gradient-to-r from-[#ff0000]/10 via-transparent to-[#ff0000]/5" />
            ...
            <div className="relative h-full flex items-center justify-center text-center text-white px-4">
              <div className="max-w-6xl">
                ...
                <div className="mb-8">
                  ...
                  <div className="w-24 h-1 bg-gradient-to-r from-[#ff0000] to-[#007a4e] rounded-full mx-auto mb-2"></div>
                  <div className="w-12 h-1 bg-gradient-to-r from-[#007a4e] to-[#ff0000] rounded-full mx-auto opacity-60"></div>
                </div>
                ...
                <div className="relative inline-block">
                  <Button className="relative bg-gradient-to-r from-[#ff0000] to-[#007a4e] hover:from-[#007a4e] hover:to-[#006640] text-white px-12 py-6 text-xl font-semibold rounded-2xl shadow-2xl hover:shadow-[0_20px_50px_rgba(0,149,94,0.4)] transition-all duration-500 hover:scale-105 group">
                    <Play className="mr-3 h-6 w-6 group-hover:scale-110 transition-transform duration-300" />
                    {t.heroButton}
                  </Button>
                  {/* Button Glow Effect */}
                  <div className="absolute inset-0 bg-gradient-to-r from-[#ff0000] to-[#007a4e] rounded-2xl blur-xl opacity-0 hover:opacity-30 transition-opacity duration-500 -z-10"></div>
                </div>
              </div>
            </div>
          </div>
      ...
      <Button
        variant="outline"
        size="icon"
        className="absolute left-6 top-1/2 -translate-y-1/2 w-14 h-14 bg-white/10 backdrop-blur-md border-white/20 text-white hover:bg-[#ff0000]/20 hover:border-[#ff0000]/40 rounded-2xl shadow-xl hover:shadow-2xl transition-all duration-300 hover:scale-110"
        onClick={prevSlide}
      >
      ...
      <Button
        variant="outline"
        size="icon"
        className="absolute right-6 top-1/2 -translate-y-1/2 w-14 h-14 bg-white/10 backdrop-blur-md border-white/20 text-white hover:bg-[#ff0000]/20 hover:border-[#ff0000]/40 rounded-2xl shadow-xl hover:shadow-2xl transition-all duration-300 hover:scale-110"
        onClick={nextSlide}
      >
        ...
        {slides.map((_, index) => (
          <button
            ...
            className={`relative transition-all duration-300 ${
              index === currentSlide
                ? "w-12 h-4 bg-gradient-to-r from-[#ff0000] to-[#007a4e] rounded-full shadow-lg"
                : "w-4 h-4 bg-white/50 hover:bg-white/70 rounded-full hover:scale-110"
            }`}
            onClick={() => goToSlide(index)}
          >
            {index === currentSlide && (
              <div className="absolute inset-0 bg-gradient-to-r from-[#ff0000] to-[#007a4e] rounded-full blur-md opacity-50"></div>
            )}
          </button>
        ))}
      </div>
      ...
      <div className="absolute bottom-0 left-0 w-full h-1 bg-white/20">
        <div
          className="h-full bg-gradient-to-r from-[#ff0000] to-[#007a4e] transition-all duration-300"
      ...
      <div className="absolute top-8 left-8 text-white">
        <div className="flex items-center space-x-3">
          <div className="w-12 h-12 bg-gradient-to-br from-[#ff0000] to-[#007a4e] rounded-xl flex items-center justify-center shadow-xl">
      ...
      {/* Floating Elements */}
      <div className="absolute top-1/4 right-12 w-2 h-2 bg-[#ff0000] rounded-full animate-pulse"></div>
      <div
        className="absolute top-1/3 right-8 w-1 h-1 bg-white rounded-full animate-pulse"
        style={{ animationDelay: "1s" }}
      ></div>
      <div
        className="absolute bottom-1/4 left-12 w-2 h-2 bg-[#ff0000]/60 rounded-full animate-pulse"
